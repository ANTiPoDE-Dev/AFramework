...
namespace AFramework{
    ...
    template <class T> uint32_t AList<T>::removeAll(const T & item){
        ...
        /*  se il numero di occorrenze è nullo                                  */
        if(!m_occ){
        /*  ritorno zero                                                        */
            return 0;
        }
        /*  provo ad allocare il nuovo vettore                                  */
        m_new = new T[this->m_size - m_occ];
        ...
        /*  se invece non ho abbastanza memoria ritorno 0 (non ho eliminato     */
        /*  nulla)                                                              */
        return 0;
    }
    ...
    template <class T> bool AList<T>::insert(const T& item, const uint32_t& index){
        ...
        /*  in questo caso la memoria è finita per cui ritorno false            */
        return false;
    }
    ...
    template <class T> AList<T> * AList<T>::clone() const{
        AList<T> * m_new = new AList<T>();
        /*  se l'allocazione è andata a buon fine                               */
        if(m_new){
            /*  se la lista non è vuota                                         */
            if(this->m_size){
                /*  alloco il nuovo vettore                                     */
                m_new->m_list = new T[this->m_size - 1];
                /*  se l'allocazione è andata a buon fine                       */
                if(m_new->m_list){
                    /*  effettuo la copia                                       */
                    for(uint32_t i = 0; i < this->m_size; i++){
    ...
    template <class T> ALinkedList<T> * ALinkedList<T>::clone() const{
        ...
        m_lst = new ALinkedList<T>();
        /*  se la lista è stata correttamente allocata                          */
        if(m_lst){
